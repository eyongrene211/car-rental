// components/RentCarCard/RentCarCard.jsx
import { Cog, Fuel, MapPin, Star, Users } from 'lucide-react'
import Image                              from 'next/image'
import React                              from 'react'

const RentCarCard = ({ data }) => {
  return (
        ...
        <div className='w-full flex flex-col gap-3'>
          ...
          <div className='flex items-center justify-between w-full pt-2'>
            <div className='flex items-baseline gap-1'>
              <span className='text-2xl sm:text-3xl font-bold text-gray-900'>${data.price}</span>
              <span className='text-sm sm:text-base text-gray-500'>/{data.period}</span>
            </div>
